...
function ReviewBoundary({ children }: { children: string }) {
    return (
        <Notice tone="warning" title="Prototype boundary">
            {children}
        </Notice>
    );
}
        ...
        <div className="sz-grid-3">
            <Card>
                <CardContent>
                    ...
                    <h3 className="sz-section-title" style={{ marginTop: '0.75rem' }}>
                        Your profile
                    </h3>
                    <p className="sz-copy">
                        {readiness.displayName ?? readiness.display_name ?? 'Profile saved'} ·{' '}
                        {readiness.areaCode ?? readiness.area_code ?? 'Tagudin'}
                    </p>
                </CardContent>
            </Card>
            <Card>
                <CardContent>
                    <Badge tone={provider ? 'success' : 'warning'}>
                        {provider ? 'Enabled' : 'Setup needed'}
                    </Badge>
                    <h3 className="sz-section-title" style={{ marginTop: '0.75rem' }}>
                        Offer capability
                    </h3>
                    <p className="sz-copy">
                        {provider
                            ? 'You can create and manage a listing.'
                            : 'Complete setup before creating a listing.'}
                    </p>
                </CardContent>
            </Card>
            <Card>
                <CardContent>
                    <Badge tone={draft?.status === 'pending_review' ? 'warning' : 'info'}>
                        {draft?.status === 'pending_review' ? 'Pending review' : 'Not public yet'}
                    </Badge>
                    <h3 className="sz-section-title" style={{ marginTop: '0.75rem' }}>
                        Publication gate
                    </h3>
                    <p className="sz-copy">Listings stay private until review accepts them.</p>
                </CardContent>
            </Card>
        </div>
    );
...
function ListingWorkspace({
    ...
    const [category, setCategory] = useState(
        draft?.categoryCode ?? draft?.category_code ?? 'home-help',
    );
            ...
            <CardContent className="sz-stack">
                <div className="sz-row-between">
                    <div>
                        ...
                        <h2 className="sz-section-title">
                            Create something useful for your neighbors.
                        </h2>
                        <p className="sz-copy">
                            Drafts stay private. You can preview the public shape before submitting
                            for review.
                        </p>
                    </div>
                    {draft ? (
                ...
                <form onSubmit={save} className="sz-grid-2">
                    <div className="sz-stack">
                        <Field
                            label="Listing title"
                            error={errors.title}
                            hint="Use a clear, human title. No claims of verification. "
                        >
                            <TextInput
                                name="title"
                                value={title}
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.65rem' }}>
                            <Button type="submit" loading={action === 'listing'}>
                                {draft ? 'Save private draft' : 'Create private draft'}
                            </Button>
                            {draft ? (
                                <Button
                                    type="button"
...
function WorkspaceState({
    ...
    const createDraft = (input: {
        ...
        router.post('/listings', input, {
            preserveScroll: true,
            onSuccess: () =>
                setListingNotice('Draft created. The server projection is now authoritative.'),
        });
    };
    const saveDraft = (input: {
        ...
        router.patch(
            `/listings/${draft.id}`,
            {
                ...input,
                expected_version:
                    draft.version ?? draft.expectedVersion ?? draft.expected_version ?? 1,
            },
            {
                preserveScroll: true,
                onSuccess: () =>
                    setListingNotice('Draft saved. Refresh or continue from the server version.'),
            },
        );
    };
    const submitDraft = () => {
    ...
    return (
        <ProductShell
            ...
            active={listingOnly ? 'listings' : 'home'}
            title={listingOnly ? 'My Listings' : 'Your local workspace'}
        >
            <main className="sz-page">
                <div className="sz-row-between" style={{ marginBottom: '1.25rem' }}>
                    <div>
                        <p className="sz-eyebrow">
                            {listingOnly
                                ? 'Private workspace'
                                : `Good to see you, ${displayName(props.session, readiness)}`}
                        </p>
                        <h1 className="sz-display-title">
                            {listingOnly ? 'My Listings' : 'Make your next useful move.'}
                        </h1>
                        <p className="sz-copy">
                            {listingOnly
                                ? 'Save drafts, review the buyer preview, and follow each server-owned publication state.'
                                : 'Your workspace keeps setup, listings, and review status in one place.'}
                        </p>
                    </div>
                    <Button
                        type="button"
                ...
                <ReviewBoundary>
                    Drafts and pending review stay private. Only approved active listings appear in
                    public browse.
                </ReviewBoundary>
                {!listingOnly ? (
                    <section style={{ marginTop: '1.25rem' }}>
                        <StatusSummary readiness={readiness} draft={draft} />
                    </section>
                ) : null}
                {notice ? (
                ...
                {!listingOnly ? (
                    <section style={{ marginTop: '1.25rem' }}>
                        <Card>
                            <CardContent>
                                <p className="sz-eyebrow">Browse</p>
                                <h2 className="sz-section-title">See what is already active.</h2>
                                <p className="sz-copy">
                                    Public browse is intentionally separate from your private draft
    ...
    const logout = () =>
        withAction('logout', () =>
            router.post(
                '/auth/logout',
                {},
                { preserveScroll: true, onFinish: () => setAction(null) },
            ),
        );
    ...
    return (
        <ProductShell title="Welcome to Serbizyu">
            <main className="sz-page">
                <Card>
                    <CardContent>
                        <p className="sz-eyebrow">Your marketplace</p>
                        <h1 className="sz-display-title">
                            Discover local listings built on clear information.
                        </h1>
                        <p className="sz-copy">
                            Browse what is active, or sign in securely to create and manage your own
                            listings.
                        </p>
                        <a className="sz-btn sz-btn-primary" href="/auth/sign-in">
                            Sign in securely
                        </a>
                    </CardContent>
                </Card>
